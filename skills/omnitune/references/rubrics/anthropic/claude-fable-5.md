...
## Scaffolding additions (long asynchronous agents)
- **send-to-user tool** — for long async runs, a client-side tool that surfaces verbatim user-facing content (a deliverable, a direct answer) mid-turn without ending the turn. Defining it isn't enough — pair it with an elicitation instruction or Fable 5 rarely calls it; never route narration through it. `[PF]`
- **Don't end on a promise** — deep in a long session Fable 5 may end a turn with a text-only "I'll now run X" instead of the tool call, or ask permission it doesn't need. For autonomous pipelines add an end-of-turn check (finish any planned or promised work before ending) plus a checkpoint rule: pause only for a destructive/irreversible action, real scope change, or user-only input. `[PF]`

## Delegation defaults (Mode C teams)
When Fable 5 runs a role in a Mode C team, this is its fan-out posture (the tier layer in `references/delegation-tiers.md` sets who runs what):
- **Tier position:** frontier — orchestrate long-horizon, ambiguous, multi-day/async programs; also a strong hard-auditor via fresh-context vision verifiers. `[PF]`
- **Fan-out: MORE, not fewer.** Fable 5 "dispatches parallel subagents more readily" and sustains parallel subagents with ongoing communication; steer toward frequent delegation, not restraint. This reverses Opus 4.8. `[PF]`
- **Dispatch: async over blocking.** Prefer asynchronous orchestrator↔subagent communication over blocking until each subagent returns; a Fable-5 pack should not inherit Opus-era collect-all-then-one-wave throughput barriers (the correctness invariant — one writer per file/branch on isolated worktrees — still holds). `[PF]`
- **Workers: long-lived, context-holding.** Keep long-lived subagents that hold context across subtasks (cache-read savings; no bottleneck on the slowest); do not force disposable-only workers. `[PF]`
- **Guard the emitted pack:** never write "show/echo/narrate your reasoning" into an agent definition (refusal trigger); add the long-async affordances (send-to-user, don't-end-on-a-promise, final-response re-grounding) for autonomous roles. `[PF]`

## Severity emphasis for Mode A on this model
**Highest-yield findings (Fable-5-specific):** (1) any "show/echo your reasoning" instruction → HIGH (refusal trigger); (2) over-prescriptive, enumerate-every-case skills → MEDIUM/HIGH (degrades output — recommend trimming); (3) synchronous/blocking harness assumptions for long runs, incl. "spawn fewer subagents" guidance carried over from Opus; (4) a user-facing long-run skill with no final-response register addendum, or a long-horizon agent with no memory/lesson-file practice → MEDIUM; (5) UI work relying on self-critique instead of a fresh-context vision verifier → MEDIUM; (6) a long-async-agent skill with no verbatim send-to-user path, or an autonomous-pipeline prompt with no end-of-turn "don't end on a promise" check → LOW. Findings (4)–(6) are missing *structural affordances* (a file, a tool, a brief addendum, a verifier) — orthogonal to (2)'s per-case over-enumeration, so both can apply to one skill without conflict. Note: `_core` rules still apply, but where this file reverses one, **this file wins.**
