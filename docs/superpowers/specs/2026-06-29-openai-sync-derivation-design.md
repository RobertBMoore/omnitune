...
- Match exact-or-dotted-subdomain against the provider's `allowlist_domains`: `h == d or h.endswith("." + d)`. This allows true subdomains of an allowlisted domain (and, since the manifest allowlists bare `openai.com`, any `*.openai.com`) while blocking look-alikes (`notopenai.com`) and suffix-spoofs (`openai.com.evil.com`, `developers.openai.com.evil.com`).
...
- **`allowed()` truth table (shared, identical rows in `test_tuner_check.py`):** allow `https://developers.openai.com/x`, `https://cookbook.openai.com/x`, `https://platform.openai.com/x` (bare-`openai.com` subdomain), a sub-subdomain `https://a.developers.openai.com/x`; deny `http://…` (scheme), `https://developers.openai.com@evil.com/…` (userinfo), `https://notopenai.com` (look-alike), `https://openai.com.evil.com` (suffix-spoof), `https://github.com/openai/codex` (off-allowlist), a cross-provider host (anthropic URL under `provider=openai`), IDN/case variants.
