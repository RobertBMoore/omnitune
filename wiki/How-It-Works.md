...
It first asks numbered questions for any missing launch fact: deploy targets, gate commands and required environments, checkpoint ownership/channel, quiet hours, milestone shape, and target directory — plus the team-design facts (scale, per-role runtime models, workstream independence) that size and tier the team. It never silently fills those gaps. Every project-specific detail in the result must be cited to the brief/config or listed as an assumption.

A complete pack contains seven components: a goal prompt, a short auto-loaded constitution, agent definitions derived from the brief's workstreams and domains, resumable state-file contracts, a guardrails digest, an operator pre-flight checklist, and runnable record/liveness scripts. Each agent's model and effort come from the delegation-tier layer (keyed to the model that role runs on, which may span providers); each runtime model's rubric sets its fan-out posture, effort, and verbosity; project facts come from the brief or config.

Before handoff, Mode C runs both self-checks — the recording check (all required components, the fabrication ledger, contract traceability, line caps, Python/Bash gate syntax) and the topology check (every agent tiered and justified, roles mapping to the brief, fan-out matching each runtime rubric). An unresolved check is never treated as ready. The pack does not execute the project or approve launch; it makes the operating contract explicit and reviewable, with scriptable invariants gated. See [Tune-Goal](Tune-Goal.md) for best-use guidance and a full example.
