...
- **Draft** output paths, and **default `output.packs` on** (e.g. `docs/orchestration/`) so a Mode C pack corpus exists to review and regress — omit it only if the user declines; note candidate `house_rules` / `reserved_decisions` files.
