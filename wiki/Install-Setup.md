...
## Verify
Run `/omnitune:tune-prompt "a rough prompt"`, `/omnitune:tune-skill <a skill file>`, and `/omnitune:tune-goal "Use docs/project-brief.md; save under docs/orchestration/"`. Without config all three select the session model's rubric; the Mode C example writes its pack to the named destination. Config adds routing, context pointers, and default output locations. See [Tune-Goal](Tune-Goal.md) for the brief and team-design inputs, [Configuration](Configuration.md) to hand-edit fields, and [FAQ](FAQ.md) if a command does not fire.
